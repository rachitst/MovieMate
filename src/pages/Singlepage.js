...
const Singlepage = () => {
  ...
  const handleBookTicket = () => {
    if (singleShow.status === "Ended") {
      alert("Sorry, this show has ended.");
    } else {
      ...
      {loading ? (
        ...
        <div className="singleshow">
          ...
          <div className="singleshow__info">
            ...
            <button id="myButton" onClick={handleBookTicket} className={showBookingForm ? 'clicked' : ''}>Book Ticket</button>
          </div>
        </div>
      )}
      {showBookingForm && (
        <TicketBookingForm
          movieName={singleShow.name}
